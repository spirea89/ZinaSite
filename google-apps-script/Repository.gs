...
function appendRecord_(sheetName, record, htmlFields) {
  ...
  const row = context.headers.map(function (header) {
    ...
    return (htmlFields || []).indexOf(header) !== -1 ? String(value || '') : safePlainCell_(value);
  });
...
function updateRecord_(sheetName, id, changes, htmlFields) {
  ...
  Object.keys(changes).forEach(function (header) {
    ...
    cell.setValue((htmlFields || []).indexOf(header) !== -1 ? String(changes[header] || '') : safePlainCell_(changes[header]));
  });
}
...
function withWriteLock_(callback) {
  ...
  if (!lock.tryLock(30000)) throw apiError_('BUSY', 'The CMS is busy. Try again shortly.');
...
function categoryFromRow_(row) { return { id: String(row.id), slug: String(row.slug), name_ro: String(row.name_ro), name_en: String(row.name_en), name_de: String(row.name_de), created_at: isoString_(row.created_at) }; }
...
function createArticle_(value) { return withWriteLock_(function () { ensureCategoryExists_(value.categoryId); const id = newId_(), now = nowIso_(); appendRecord_('Articles', { id:id,title:value.title,content:value.content,title_en:value.titleEn,content_en:value.contentEn,title_de:value.titleDe,content_de:value.contentDe,category_id:value.categoryId,status:value.status,created_at:now,updated_at:now }, ['content','content_en','content_de']); return findById_(listArticles_(false), id); }); }
function updateArticle_(id, value) { return withWriteLock_(function () { if(Object.prototype.hasOwnProperty.call(value,'categoryId'))ensureCategoryExists_(value.categoryId); const map={title:'title',content:'content',titleEn:'title_en',contentEn:'content_en',titleDe:'title_de',contentDe:'content_de',categoryId:'category_id',status:'status'}, changes={updated_at:nowIso_()}; Object.keys(value).forEach(function(k){changes[map[k]]=value[k];}); updateRecord_('Articles',id,changes,['content','content_en','content_de']); return findById_(listArticles_(false),id); }); }
function createCategory_(value) { return withWriteLock_(function(){ if(listCategories_().some(function(c){return c.slug===value.slug;}))throw apiError_('VALIDATION_ERROR','Category slug already exists.'); const id=newId_(); appendRecord_('ArticleCategories',{id:id,slug:value.slug,name_ro:value.nameRo,name_en:value.nameEn,name_de:value.nameDe,created_at:nowIso_()}); return findById_(listCategories_(),id); }); }
function updateCategory_(id,value){return withWriteLock_(function(){if(value.slug&&listCategories_().some(function(c){return c.slug===value.slug&&c.id!==id;}))throw apiError_('VALIDATION_ERROR','Category slug already exists.');const map={slug:'slug',nameRo:'name_ro',nameEn:'name_en',nameDe:'name_de'},changes={};Object.keys(value).forEach(function(k){changes[map[k]]=value[k];});updateRecord_('ArticleCategories',id,changes);return findById_(listCategories_(),id);});}
function deleteCategory_(id){return withWriteLock_(function(){if(readRows_('Articles').some(function(r){return String(r.category_id)===id;}))throw apiError_('CATEGORY_IN_USE','Category is referenced by an article.');deleteRecord_('ArticleCategories',id);return true;});}
function createEvent_(value){return withWriteLock_(function(){const id=newId_(),now=nowIso_();appendRecord_('Events',{id:id,title:value.title,description:value.description,title_en:value.titleEn,description_en:value.descriptionEn,title_de:value.titleDe,description_de:value.descriptionDe,start_date:value.startDate,end_date:value.endDate,location:value.location,registration_url:value.registrationUrl,status:value.status,created_at:now,updated_at:now});return findById_(listEvents_(false),id);});}
function updateEvent_(id,value){return withWriteLock_(function(){const map={title:'title',description:'description',titleEn:'title_en',descriptionEn:'description_en',titleDe:'title_de',descriptionDe:'description_de',startDate:'start_date',endDate:'end_date',location:'location',registrationUrl:'registration_url',status:'status'},changes={updated_at:nowIso_()};Object.keys(value).forEach(function(k){changes[map[k]]=value[k];});updateRecord_('Events',id,changes);return findById_(listEvents_(false),id);});}
function createTeam_(value){return withWriteLock_(function(){const id=newId_(),now=nowIso_();appendRecord_('TeamMembers',{id:id,name:value.name,role_en:value.roleEn,role_ro:value.roleRo,role_de:value.roleDe,bio_en:value.bioEn,bio_ro:value.bioRo,bio_de:value.bioDe,image_url:value.imageUrl,drive_file_id:value.driveFileId,sort_order:value.sortOrder,created_at:now,updated_at:now});return findById_(listTeam_(),id);});}
function updateTeam_(id,value){return withWriteLock_(function(){const map={name:'name',roleEn:'role_en',roleRo:'role_ro',roleDe:'role_de',bioEn:'bio_en',bioRo:'bio_ro',bioDe:'bio_de',imageUrl:'image_url',driveFileId:'drive_file_id',sortOrder:'sort_order'},changes={updated_at:nowIso_()};Object.keys(value).forEach(function(k){changes[map[k]]=value[k];});updateRecord_('TeamMembers',id,changes);return findById_(listTeam_(),id);});}
function updateHomepage_(value,admin){return withWriteLock_(function(){const changes={content:value.content,hero_image_url:value.heroImageUrl,hero_drive_file_id:value.heroDriveFileId,hero_image_position_x:value.heroImagePositionX,hero_image_position_y:value.heroImagePositionY,updated_at:nowIso_(),updated_by:admin.email};const rows=readRows_('HomepageContent').filter(function(r){return String(r.id)==='home';});if(rows.length>1)throw apiError_('INTERNAL_CONFIGURATION','Duplicate homepage rows exist.');if(rows.length)updateRecord_('HomepageContent','home',changes,['content']);else appendRecord_('HomepageContent',Object.assign({id:'home'},changes),['content']);return homepageContent_();});}
